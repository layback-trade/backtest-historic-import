interface CalculateOddGapRequest {
  minOdd: number
  maxOdd: number
}
...
export const odds = [
  ...
  28, 29, 30, 31, 32, 34, 36, 38, 40, 42, 44, 46, 48, 50, 55, 60, 65, 70, 75,
...
export function calculateOddGap({ minOdd, maxOdd }: CalculateOddGapRequest) {
  // find de closest odd index to the minOdd
  const minOddIndex = odds.findIndex((odd) => odd >= minOdd)

  // find de closest odd index to the maxOdd
  const maxOddIndex = odds.findIndex((odd) => odd >= maxOdd)

  // calculate the gap between the minOdd and the maxOdd
  const gap = Math.abs(maxOddIndex - minOddIndex)
